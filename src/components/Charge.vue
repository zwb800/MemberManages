    ...
    <q-card style="min-width: 700px">
      ...
      <q-form @submit="submit">
        ...
        <q-card-actions align="right">
          <span>{{ text ? '应付金额:' : '' }}</span
          ><span class="text-negative text-bold">{{ text }}</span
          >&nbsp;
          <q-btn
            :loading="submitting"
            label="充值"
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    watch([amount, card], () => {
      text.value = '';
      if (amount.value && amount.value > 0) text.value += `+${amount.value}`;
      if (card.value) text.value += `+${card.value?.price}`;
    ...
    const submit = async () => {
      ...
      let amountV = 0;
      if (amount.value) amountV = parseInt(amount.value.toString());

...
<q-input stack-label v-model="dateRef" label="月份" type="date"></q-input>
  <q-markup-table dense  separator="cell">
    <thead>
      <tr>
        <th></th>
        <th :colspan="serviceItems.length" v-for="e in employees" :key="e">{{e.name}}</th>
      </tr>
      <tr>
        <th></th>
    ...
    <tbody>
      <tr v-for="r in rowsRef" :key="r">
        <td v-for="c,i in r" :key="c" :class="{'b':i%serviceItems.length==0}">
          <template v-if="c!=0">
            {{c}}
          </template>
          </td>
  ...
  <q-markup-table dense separator="cell" class="sumtable">
    <thead>
      <tr>
        <th></th>
          <th v-for="s,i in serviceItems" :key="s">{{s.shortName}}({{priceServiceItems[i]}})</th>
          <th>工作量提成</th>
          <th>办卡提成</th>
      </tr>
    </thead>
...
const cardCount = ref(0)
 ...
 const mapServiceItems = new Map<string,number>()
    ...
    for (const it of serviceItems.value) {
      mapServiceItems.set(it._id,index++)
    }
    let eIndex = 0
    for (const e of employees.value) {
      mapEmployees.set(e._id,eIndex++)
    }
    }

     const item2Index = (employeeId:string,item:string)=>{
    ...
    const rows = Array<Array<number|string>>(size+1)
        for (let i = 0; i < rows.length; i++) {
          const column = Array<number|string>(employees.value.length * serviceItems.value.length+1)
          column.fill(0)
          if(i==rows.length-1){
            column[0]= '总'
          }
...
const rows = initRow(new Date(lastConsume.time).getDate())
    for (const c of result.consumes) {
      ...
      for (const ce of c.employees) {
        for (const item of ce.items) {
          const cIndex = item2Index(ce.employeeId,item)
    ...
    for (let i = 0; i < employees.value.length; i++) {
      ...
      for (let j = 0; j < serviceItems.value.length; j++) {
        const v = sumRow[i*serviceItems.value.length+j+1]
          row.push(v)
          if(typeof(v) == 'number')
            sumPrice+= v *priceServiceItems.value[j]
      }
      row.push(sumPrice)
      row.push(0)
      sumRows.push(row)
    ...
    for (const c of result.charges) {
      ...
      if(c.employees.length>0){
        ...
        es = employees.value .filter(e=>e.shopId == c.shopId).map(p=>p._id)
      }
